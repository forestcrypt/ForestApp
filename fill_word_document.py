...
class WordDocumentFiller:
    def __init__(self, db_name='forest_data.db', data_file=None, address_data=None, total_data=None):
        ...
        # Используем правильный шаблон по образцу
        self.document_path = 'reports/Шаблон проект_правильный.docx'
    ...
    def fill_document(self):
        ...
        try:
            ...
            # Используем плейсхолдеры из идеального шаблона
            replacements = {
                # Адресные данные
                '(Адрес-лесничество)': address_data.get('forestry', ''),
                '(Адрес-участковое лесничество)': address_data.get('district_forestry', ''),
                '(Адрес-квартал)': address_data.get('quarter', ''),
                '(Адрес-выдел)': address_data.get('plot', ''),
                '(Адрес-площадь)': address_data.get('plot_area', ''),
                ...
                '(Детали-характеристика-лучшие)': best_text,
                '(Детали-характеристика-вспомогательные)': auxiliary_text,
                '(Детали-характеристика-нежелательные)': undesirable_text,
                ...
                '(Итого-густота)': format_number(self.total_data.get('avg_density')),
                ...
                '(Данных по площадкам-тип леса)': address_data.get('forest_type', forest_type),
            }
