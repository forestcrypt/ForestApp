...
# -*- coding: utf-8 -*-
"""
Тестовый запуск fill_our_template.py с ИСПРАВЛЕНИЯМИ и данными проекта
"""
...
# Извлекаем Тип Леса из данных участка (столбец 5)
forest_type = 'Смешанный лес'  # По умолчанию
for page_num, page_rows in page_data.items():
    for row in page_rows:
        if len(row) >= 6 and row[5]:  # Столбец 5 - Тип Леса (живой напочвенный покров)
            # В JSON row[4] - живой напочвенный покров, row[5] - Тип Леса
            # Но в нашем JSON только 5 колонок (0-4), поэтому Тип Леса не сохранён
            # Будем использовать "Брусничник" из анализа участка
            forest_type = 'Брусничник'
            break

# Добавляем forest_type в address_data
address_data['forest_type'] = forest_type
...
# РАСЧЁТ ИНТЕНСИВНОСТИ ПО ФОРМУЛЕ (как в get_total_data_from_db)
total_density_all_plots = sum(b['density'] for b in breeds_list)  # Суммарная густота
avg_remaining_density = 3000  # 3С = 3000 шт/га

if total_density_all_plots > 0:
    intensity = ((total_density_all_plots - avg_remaining_density) / total_density_all_plots) * 100
else:
    intensity = 25.0  # По умолчанию

print(f"[DEBUG] Расчёт интенсивности: ({total_density_all_plots:.1f} - {avg_remaining_density}) / {total_density_all_plots:.1f} = {intensity:.1f}%")
...
print(f"  forest_type: {total_data.get('forest_type', address_data.get('forest_type', 'Н/Д'))}")
print(f"  characteristics: {total_data['characteristics']}")
